...
namespace ZXWebApps.Controllers
{
    public class DashboardController : Controller
    {
        ...
        public ActionResult LastInsert()
        {
            List<Daily> dailies = new List<Daily>();
            using (HELLOWEntities db = new HELLOWEntities())
            {
                dailies = (from daily in db.tt_Daily
                 join record in db.tm_Recorder on daily.Recorder_FK equals record.Recorder_PK
                 where daily.Status_FK == 1
                 select new Daily { Daily_PK = daily.Daily_PK, Inspector = record.Nama, SheetNum = daily.SheetNum, InsertDate = daily.Date }).OrderByDescending(x=>x.Daily_PK).Take(5).ToList();
            }
            return PartialView(dailies);
        }
            ...
            try
            {
                using (HELLOWEntities db = new HELLOWEntities())
                {
                    var data = (from daily in db.tt_Daily
                                   ...
                                   select new Daily { Daily_PK = daily.Daily_PK, InsertDate = daily.Date }).FirstOrDefault();
        ...
        public ActionResult Edit(int id = 0)
        {
            ...
            using (HELLOWEntities db = new HELLOWEntities())
            {
                ...
                edited.Date = edited.Datetemp.ToString("dd MMMM yyyy");
